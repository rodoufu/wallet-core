...
TEST(NEOTransaction, validity) {
    ...
    uint64_t amount = 1;
    std::list<boost::any> transferParam{fromAddress.bytes, toAddress.bytes, amount};
    ...
    std::string hexTx =
        "00d1e3388d5c5802000000000000e0930400000000001757e9d1a61f9aafa798b6c7fbeae35639681d7df66e00"
        "c66b1517fbacc8214765d457c8e3f2b5a1d3c4981a2e9d2a6a7ccb151757e9d1a61f9aafa798b6c7fbeae35639"
        "681d7df66a7ccb516a7ccb6c51c1087472616e7366657214000000000000000000000000000000000000000100"
        "68114e454f2e4e61746976652e496e766f6b650000";
    ...
    EXPECT_EQ(516, hex(tx.serialize()).length());
    ...
    EXPECT_EQ(446, verifyPosition1);
    EXPECT_EQ(650, verifyPosition2);
    EXPECT_EQ(720, hex(result).length());
}
