...
class ParamsBuilder {
  ...
  public:
    ...
    static void buildNeoVmParam(ParamsBuilder& builder, const std::array<uint8_t, AddressSize>& param);
    ...
    void push(const std::array<uint8_t, AddressSize>& data);
    ...
    void pushBack(const std::array<uint8_t, AddressSize>& data);
    template <typename T>
