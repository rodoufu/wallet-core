...
void ParamsBuilder::buildNeoVmParam(ParamsBuilder& builder, const boost::any& param) {
    ...
    } else if (param.type() == typeid(std::array<uint8_t, AddressSize>)) {
        builder.push(boost::any_cast<std::array<uint8_t, AddressSize>>(param));
...
void ParamsBuilder::buildNeoVmParam(ParamsBuilder& builder, const std::array<uint8_t, AddressSize>& param) {
...
void ParamsBuilder::push(const std::array<uint8_t, AddressSize>& data) {
    push(Data(data.begin(), data.end()));
}
...
void ParamsBuilder::pushBack(const std::array<uint8_t, AddressSize>& data) {
    bytes.insert(bytes.end(), data.begin(), data.end());
}
